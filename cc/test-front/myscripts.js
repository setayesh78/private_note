...
async function sendToBaseUrl() {
    ...
    fetch('http://192.168.49.102:8000/note/add/', requestOptions)
    .then(response => response.text())
    .then(result => link = result)
    .catch(error => console.log('error', error));

    setTimeout(() => { console.log(link); }, 20000);

    setTimeout(() => { url_a = 'index1.html?name=' + link; }, 20500);

    setTimeout(() => { window.location.href = url_a; }, 21000);
...
function showNote() {
    ...
    console.log(test)
     ...
     fetch('http://192.168.49.102:8000/note/delete/'+test)
    .then(response => response.text())
